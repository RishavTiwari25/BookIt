...
async function ensureSchema() {
    try {
        ...
        const cols = (await prisma.$queryRawUnsafe(`PRAGMA table_info("Booking");`));
        const hasQuantity = Array.isArray(cols) && cols.some((c) => c.name === 'quantity');
        const hasContactPhone = Array.isArray(cols) && cols.some((c) => c.name === 'contactPhone');
        const hasPassengers = Array.isArray(cols) && cols.some((c) => c.name === 'passengers');
        if (!hasQuantity) {
...
app.post("/bookings", async (req, res) => {
    ...
    if (!Number.isFinite(peopleCount) || peopleCount < 1)
        return res.status(400).json({ error: "At least 1 passenger required" });
    ...
    if (Array.isArray(passengers)) {
        for (const p of passengers) {
            ...
            if (!Number.isFinite(a) || a < 0 || a > 120)
                return res.status(400).json({ error: "Passenger age invalid" });
            if (!['M', 'F', 'O'].includes(s))
                return res.status(400).json({ error: "Passenger sex must be M, F, or O" });
        }
    }
    try {
      ...
      `;
            ...
            if (promoCode) {
                ...
                if (code && code.active) {
                    ...
                    if (finalPrice.lessThan(0))
                        finalPrice = new finalPrice.constructor(0);
                }
            }
        ...
        if (err instanceof Error) {
            if (err.message === "SOLD_OUT")
                return res.status(409).json({ error: "Slot is sold out" });
...
ensureSchema().finally(() => {
    ...
    try {
        ...
        app.get(["/", "/details/:id", "/checkout", "/booking-result"], (_req, res) => {
